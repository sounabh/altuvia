...
// ✅ FIX 1: OpenRouter Configuration
const OPENROUTER_API_KEY = process.env.OPENROUTER_API_KEY;
const OPENROUTER_BASE_URL = "https://openrouter.ai/api/v1";
const GEMINI_MODEL = "google/gemini-2.5-flash-lite";

/**
 * ✅ FIX 1: OpenRouter API call function
 */
async function callGeminiViaOpenRouter(prompt, maxTokens = 75536) {
  try {
    ...
    const response = await fetch(`${OPENROUTER_BASE_URL}/chat/completions`, {
      ...
      headers: {
        ...
        "X-Title": "University Application Timeline Generator",
      },
      body: JSON.stringify({
        model: GEMINI_MODEL,
        messages: [
...
/**
 * ✅ FIX 1: Database retry logic with exponential backoff
 */
async function saveTimelineWithRetry(
  userId,
  universityId,
  programId,
  timeline,
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      ...
      const result = await saveTimelineToDatabase(
        userId,
        universityId,
        programId,
        timeline,
      ...
      lastError = error;
      console.error(
        `❌ Database save attempt ${attempt} failed:`,
        error.message
      );
  ...
  throw new Error(
    `Database save failed after ${maxRetries} attempts: ${lastError?.message}`
  );
}
...
function matchCalendarEventToTask(task, calendarEvents) {
  ...
  const matchedEvent = calendarEvents.find((event) => {
    ...
    if (similarity >= 0.8) {
      console.log(
        `✅ Calendar match (${Math.round(similarity * 100)}%): "${
          event.title
        }" → "${task.title}"`
      );
      return true;
...
function parseTestScores(testScoresString) {
  ...
  try {
    const parsed =
      typeof testScoresString === "string"
...
async function fetchFreshMetadata(userId, universityId) {
  try {
    ...
    const completedEssaysCount = enhancedEssays.filter(
    ...
    const completedEvents = freshCalendarEvents.filter(
      (e) => e.completionStatus === "completed"
    ).length;
    const pendingEvents = freshCalendarEvents.filter(
      (e) => e.completionStatus === "pending"
    ).length;
    const now = new Date();
    ...
    const acceptanceRate =
      freshPrograms[0]?.admissions?.[0]?.acceptanceRate || null;

    return {
      essaysRequired: totalEssayPromptsCount,
      essaysCompleted: completedEssaysCount,
      essaysRemaining: Math.max(
        0,
        totalEssayPromptsCount - completedEssaysCount
      ),
      essaysNotStarted: notStartedEssaysCount,
      essayCompletionRate:
        totalEssayPromptsCount > 0
...
function matchEssayTaskToUserEssay(taskTitle, allEssayPrompts, enhancedEssays) {
  ...
  if (
    ...
    !taskTitleLower.includes("prompt")
  ) {
    return {
      isTaskComplete: false,
      relatedEssayId: null,
      matchType: "not_essay_task",
    };
  }
...
async function getExistingTimeline(userId, universityId) {
  try {
    ...
    console.log(`Timeline Details:`, {
      ...
      totalTasks:
        existingTimeline.phases?.reduce(
    ...
    try {
      ...
      try {
        const userSnap =
          typeof existingTimeline.userProfileSnapshot === "string"
            ? JSON.parse(existingTimeline.userProfileSnapshot)
            : existingTimeline.userProfileSnapshot;
        const uniSnap =
          typeof existingTimeline.universitySnapshot === "string"
        ...
        console.error(
          "❌ Could not extract metadata from snapshots:",
          snapError.message
        );
        freshMetadata = null;
      }
  ...
  phases: existingTimeline.phases.map((phase, phaseIdx) => ({
    ...
    const completedTasks = timeline.phases.reduce(
      (sum, p) => sum + p.tasks.filter((t) => t.completed === true).length,
      0
    );
    ...
    return {
      ...
      metadata: {
        ...
        generatedAt: existingTimeline.generatedAt,
        universityName:
          universityData.universityName || existingTimeline.timelineName,
        location: universityData.location,
        deadline: universityData.deadline,
        daysUntilDeadline: universityData.deadline
          ? Math.ceil(
              (new Date(universityData.deadline) - new Date()) /
                (1000 * 60 * 60 * 24)
            )
          : null,
        acceptanceRate:
          freshMetadata.acceptanceRate || universityData.acceptanceRate,
...
async function saveTimelineToDatabase(
  ...
  try {
    console.log(
      `Saving timeline to database for user ${userId}, university ${universityId}, program ${
        programId || "NULL"
      }`
    );
    ...
    if (existingTimeline) {
      ...
      savedTimeline = await prisma.aITimeline.create({
        data: {
          ...
          aiModel: metadata.model || "gemini-2.5-flash-lite (via OpenRouter)",
          promptVersion: "2.0",
          generationTime: metadata.processingTime,
          userProfileSnapshot: JSON.stringify({
            overview: timeline.overview,
    ...
    for (
      let phaseIndex = 0;
      phaseIndex < timeline.phases.length;
      phaseIndex++
    ) {
      ...
      if (phase.tasks && phase.tasks.length > 0) {
        ...
        for (let i = 0; i < phase.tasks.length; i += batchSize) {
          ...
          await Promise.all(
            taskBatch.map(async (task, batchIndex) => {
              ...
              try {
                await prisma.timelineTask.create({
                  data: {
                    ...
                    title:
                      task.title?.substring(0, 200) || `Task ${taskIndex + 1}`,
...
async function reloadTimelineFromDatabase(timelineId) {
  try {
    ...
    console.log(
      `✅ Timeline reloaded: ${timeline.phases.reduce(
        (sum, p) => sum + p.tasks.length,
        0
      )} tasks with database IDs`
    );
    return timeline;
  } catch (error) {
 ...
 */
function validateAndFixTaskCompletion(
  timeline,
  ...
  timeline.phases?.forEach((phase, phaseIdx) => {
    phase.tasks?.forEach((task, taskIdx) => {
      ...
      if (task.completed === true && shouldBeCompleted === false) {
        console.log(
          `🔧 FIXING: "${task.title}" - AI said complete, but NO DATABASE PROOF`
        );
  ...
  console.log(
    `   Final completed count: ${timeline.phases.reduce(
...
function parseAIResponse(responseText) {
  ...
  try {
    ...
    if (phases.length > 0) {
      console.log(
        `✅ Strategy 3 (extract phases) found ${phases.length} complete phases`
      );
...
function isResponseComplete(text) {
  try {
    ...
    const phase4 = /"id"\s*:\s*4\s*,\s*"name"\s*:\s*"Recommendation/i.test(
      text
    );
    ...
    if (!hasAllPhases) {
      console.log(
        `Phase check: 1=${phase1}, 2=${phase2}, 3=${phase3}, 4=${phase4}, 5=${phase5}`
      );
      return false;
    }
    ...
    if (!hasProperEnding) {
      console.log(
        `Response doesn't have proper JSON ending. Last 50 chars: ${trimmed.substring(
          trimmed.length - 50
        )}`
      );
      return false;
...
function mergeResponses(original, continuation) {
  ...
  while (
    contStart < cleanContinuation.length &&
...
async function generateCompleteTimeline(
  ...
  while (
    ...
    console.log(
      `⚠️ Response incomplete. Missing ${missingPhases.length} phases. Requesting continuation ${continuationCount}...`
    );
    ...
    try {
      ...
      console.log(
        `Continuation ${continuationCount} length: ${continuationText.length}`
      );
      ...
      console.error(
        `Continuation ${continuationCount} failed:`,
        contError.message
      );
      break;
    }
  ...
  if (continuationCount > 0) {
    console.log(
      `✅ Timeline generation completed with ${continuationCount} continuation(s)`
    );
  }
...
/**
 * Retry wrapper for OpenRouter calls
 */
...
export async function POST(request) {
  try {
    ...
    console.log(
      `🎯 API Request for University: ${university.id} - ${
    ...
    if (!OPENROUTER_API_KEY) {
      ...
      return NextResponse.json(
        {
          error:
            "AI service not configured properly. Please check server configuration.",
        },
        { status: 500 }
      );
    }
    ...
    if (!forceRegenerate) {
      ...
      if (existingData) {
        console.log(
          "✅ Returning existing timeline with database completion state"
        );
    ...
    console.log(
      `📚 Fetching programs for user's studyLevel: ${
        userStudyLevel || "NOT SET"
      }`
    );
    ...
    console.log(
      `Found ${universityPrograms.length} programs for study level: ${userStudyLevel}`
    );

    if (universityPrograms.length === 0) {
      console.warn(
        `⚠️ No programs found for studyLevel: ${userStudyLevel} at university ${university.id}`
      );
    }
    ...
    console.log(
      `📝 Fetching essays for ${programIds.length} program(s):`,
      programIds
    );
    ...
    const allEssayPrompts = universityPrograms.flatMap(
      (p) => p.essayPrompts || []
    );
    const totalEssayPromptsCount = allEssayPrompts.length;
    const completedEssaysCount = enhancedEssays.filter(
    ...
    console.log(
      `Essay completion: ${completedEssaysCount}/${totalEssayPromptsCount} (using 98% logic)`
    );
    console.log(`Essays not started: ${notStartedEssaysCount}`);

    const essayCompletionFlags = allEssayPrompts.map((prompt, idx) => {
      const essay = enhancedEssays.find(
        (e) => e.essayPromptId === prompt.id || e.essayPrompt?.id === prompt.id
      );
    ...
    console.log(
      "📝 Essay Completion Flags for AI Prompt:",
      essayCompletionFlags
    );
    ...
    console.log(
      `Found ${userCalendarEvents.length} calendar events for this university`
    );

    const completedEvents = userCalendarEvents.filter(
      (e) => e.completionStatus === "completed"
    ).length;
    const pendingEvents = userCalendarEvents.filter(
      (e) => e.completionStatus === "pending"
    ).length;
    const now = new Date();
    ...
    const acceptanceRate =
      ...
      universityPrograms[0]?.admissions?.[0]?.acceptanceRate ||
      null;

    // ====== STEP 6: PREPARE DATA FOR AI PROMPT ======
    const universityName =
      university.universityName || university.name || "Your University";
    const location =
      university.location ||
      `${university.city || ""}, ${university.country || ""}`.replace(
        /^, |, $/g,
        ""
      ) ||
      "Location not specified";

    // ✅ NEW: Parse university round deadlines (same logic as UniversityCard)
    const parseUniversityRoundDeadlines = () => {
      let rawDeadlines = [];

      // Check roundDeadlines array first
      if (
      ...
      // Check averageDeadlines string
      else if (
        typeof university?.averageDeadlines === "string" &&
        university.averageDeadlines.trim()
      ) {
        const pattern =
          /(Round\s*\d+|Deferred):\s*([^,]+(?:,\s*\d{4})?[^R]*?)(?=\s*(?:Round\s*\d+|Deferred):|$)/gi;
        ...
        if (matches.length > 0) {
          ...
          rawDeadlines = university.averageDeadlines
            .split(/,(?=\s*(?:Round|Deferred))/i)
            .map((p) => p.trim())
            .filter(Boolean);
        }
      }

      // Parse dates and filter future deadlines only
      const now = new Date();
      ...
      rawDeadlines.forEach((deadline, idx) => {
        ...
        // Only include if it's a valid future date
        if (!isNaN(parsedDate.getTime()) && parsedDate > now) {
          parsedDeadlines.push({
            date: parsedDate,
            dateString: dateStr,
            round: round,
            source: "university_rounds",
            priority: "high",
          });
        }
      });
    ...
    // Get round deadlines from university data
    const roundDeadlines = parseUniversityRoundDeadlines();

    // Get deadlines from database programs
    ...
    // Get deadlines from calendar (optional - for user-created events)
    ...
    // ✅ PRIORITY ORDER: University rounds > Database deadlines > Calendar events
    const allDeadlines = [
      ...roundDeadlines,
      ...allDeadlinesFromDB,
      ...calendarDeadlines,
    ...
    // ✅ Get the NEAREST deadline (first future deadline)
    const mainDeadline = allDeadlines[0]?.date || null;
    const mainDeadlineSource = allDeadlines[0]?.source || null;
    const mainDeadlineRound = allDeadlines[0]?.round || null;
    ...
    console.log("📅 Deadline Analysis:", {
      ...
      roundDeadlines: roundDeadlines.length,
      dbDeadlines: allDeadlinesFromDB.length,
      calendarDeadlines: calendarDeadlines.length,
      nearestDeadline: mainDeadline
        ? new Date(mainDeadline).toLocaleDateString()
        : "None",
      source: mainDeadlineSource,
      round: mainDeadlineRound,
      daysUntil: daysUntilDeadline,
    });

    const deadlinesList = allDeadlines
      .slice(0, 10)
      .map((d, i) => {
        const roundInfo = d.round ? ` (${d.round})` : "";
        return `${i + 1}. ${
          d.title || d.round || "Deadline"
        }${roundInfo} - ${new Date(d.date).toLocaleDateString("en-US", {
          month: "long",
          day: "numeric",
          year: "numeric",
        })} [${d.priority || "normal"} priority] [Source: ${d.source}]`;
      })
      .join("\n");
      ...
      .map((e, i) => {
        const status =
          e.completionStatus === "completed"
            ...
            ? "⚠️ OVERDUE"
            : "📅";
        return `${i + 1}. [${status}] ${e.title} - ${e.eventType} - ${new Date(
          e.startDate
        ).toLocaleDateString()} (${e.priority} priority)${
    ...
    const testProgress =
      testNeeded.length === 0
        ? 100
        : Math.round(
            (testStatus.length / (testStatus.length + testNeeded.length)) * 100
          );
    const overallProgress = Math.round(
      (essayProgress + eventProgress + testProgress) / 3
    );
...
🗓️ Main Deadline: ${
      mainDeadline
        ? new Date(mainDeadline).toLocaleDateString("en-US", {
            month: "long",
            day: "numeric",
            year: "numeric",
...
${
  mainDeadlineSource === "university_rounds" && mainDeadlineRound
    ? `📊 Round: ${mainDeadlineRound}`
    : ""
}
📈 Acceptance Rate: ${acceptanceRate ? acceptanceRate + "%" : "N/A"}
💰 Application Fee: ${
      applicationFee ? applicationFee + " " + currency : "Check website"
    }
...
• Work Experience: ${hasWorkExp ? "Yes" : "No"}${
      needsWorkExp ? " (REQUIRED by program)" : ""
    }
...
${essayCompletionFlags
  .map(
    (flag, idx) =>
      `Essay #${idx + 1}: completed: ${flag} ${
        flag === "true"
          ? "← ✅ USER COMPLETED THIS"
          : "← ❌ NOT STARTED - SET completed: false"
      }`
  )
  .join("\n")}
...
${
  allEssayPrompts
    .map(
      (p, i) =>
        `${i + 1}. "${p.promptTitle}" - ${p.wordLimit} words - ${
          p.isMandatory ? "REQUIRED" : "Optional"
        }\n   Prompt: ${p.promptText?.substring(0, 150)}...`
    )
    .join("\n\n") || "No essay prompts available"
}

📅 STUDENT'S CALENDAR EVENTS (${completedEvents}/${
...
• GMAT Min: ${admissionReqs.gmatMinScore || "N/A"}${
      requiresGMAT ? " (REQUIRED)" : ""
    }
• GRE Min: ${admissionReqs.greMinScore || "N/A"}${
      requiresGRE ? " (REQUIRED)" : ""
    }
• IELTS Min: ${admissionReqs.ieltsMinScore || "N/A"}${
      requiresIELTS ? " (REQUIRED)" : ""
    }
• TOEFL Min: ${admissionReqs.toeflMinScore || "N/A"}${
      requiresTOEFL ? " (REQUIRED)" : ""
    }
...
${
  testNeeded.length > 0
    ? `- MUST COMPLETE: ${testNeeded.join(", ")}`
    : "- Tests already done - focus on score submission"
}
...
- Pay application fee (${
      applicationFee ? applicationFee + " " + currency : "amount TBD"
    })
- Triple-check every section for errors
${
  mainDeadline
    ? `- Submit 5-7 days BEFORE deadline (${new Date(
        mainDeadline
      ).toLocaleDateString()}${
        mainDeadlineRound ? ` - ${mainDeadlineRound}` : ""
      })`
    : "- Submit application"
}
...
Generate the complete timeline JSON now:`;
    ...
    // ====== STEP 8: CALL AI API VIA OPENROUTER WITH CONTINUATION SUPPORT ======
    let timelineData;

    try {
      console.log(`📤 Sending prompt to Gemini via OpenRouter...`);
      const responseText = await generateCompleteTimeline(prompt, 2, 2);
      console.log("Final AI Response length:", responseText.length);

      try {
        timelineData = parseAIResponse(responseText);
        console.log(
          `✅ AI timeline parsed successfully with ${
    ...
    if (timelineData.phases.length < 4) {
      console.error(
        `❌ AI generated only ${timelineData.phases.length} phases`
      );
      return NextResponse.json(
        {
    ...
    if (timelineData.phases.length === 4) {
      ...
      const phaseNames = timelineData.phases.map(
        (p) => p.name?.toLowerCase() || ""
      );

      const hasSubmissionPhase = phaseNames.some(
        (n) =>
      ...
      if (!hasSubmissionPhase) {
        console.log(
          "Adding placeholder Phase 5: Application Assembly & Submission"
        );
    ...
    console.log(
      `✅ Validated: Timeline has ${timelineData.phases.length} phases`
    );
    ...
    const totalTasks = timelineData.phases.reduce(
      (sum, p) => sum + (p.tasks?.length || 0),
      0
    );
    const completedWithProof = timelineData.phases.reduce(
      (sum, p) =>
        sum + (p.tasks?.filter((t) => t.completed === true).length || 0),
      0
    );
    ...
    const metadata = {
      ...
      deadlineSource: mainDeadlineSource,
      deadlineRound: mainDeadlineRound,
      acceptanceRate: acceptanceRate,
      applicationFee: applicationFee,
      allDeadlines: allDeadlines, // ← ADD THIS LINE
      deadlinesList: deadlinesList, // ← ADD THIS LINE TOO
      ...
      processingTime: processingTime,
      model: `${GEMINI_MODEL} (via OpenRouter)`,
    ...
    try {
      ...
      if (reloadedTimeline) {
        ...
        console.log(
          "✅ Timeline replaced with database version (has proper IDs and completion status)"
        );
      } else {
        console.warn(
          "⚠️ Could not reload timeline from database, using AI-generated version"
        );
      }
    } catch (dbError) {
      console.error(
        "❌ Failed to save timeline to database after retries:",
        dbError.message
      );
      savedToDatabase = false;
      metadata.dbError = dbError.message;
      metadata.warningMessage =
        "Timeline generated but could not be saved to database. Task completion will not persist.";
    }
    ...
    console.log("📤 Sending response:", {
      ...
      totalTasks: timelineData.phases.reduce(
        (sum, p) => sum + (p.tasks?.length || 0),
        0
      ),
      ...
      tasksWithDatabaseIds: timelineData.phases.reduce(
        (sum, p) =>
          sum +
          (p.tasks?.filter(
            (t) => t.id && typeof t.id === "string" && t.id.length > 10
          ).length || 0),
        0
      ),
