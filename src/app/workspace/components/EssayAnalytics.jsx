...
// EssayAnalytics component receives an individual essay and all essays for stats
export function EssayAnalytics({ essay, allEssays }) {
  // Completion percentage of current essay
  ...
  // Writing velocity (words per day since last modified)
  const daysSinceStart = Math.max(
    1,
    Math.ceil(
      (new Date().getTime() - essay.lastModified.getTime()) /
        (1000 * 60 * 60 * 24)
    )
  )
  ...
  const avgSentenceLength =
    sentences.length > 0
      ? Math.round(essay.wordCount / sentences.length)
      : 0
  ...
  // Average progress across all essays
  const totalProgress =
    allEssays.reduce(
      (acc, e) =>
        acc + Math.min(100, (e.wordCount / e.wordLimit) * 100),
      0
    ) / allEssays.length

  // Helper function to return color classes based on priority
    ...
    <Card className="shadow-xl border-0 bg-white/70 backdrop-blur-sm">
      <div className="p-6">
        ...
        <div className="space-y-6">
          ...
          {/* Writing Metrics: Reading Time and Velocity */}
          ...
          {/* Quick Stats: Sentences, Paragraphs, Last Updated */}
          <div className="grid grid-cols-1 gap-2 text-xs">
            ...
            <div className="flex justify-between items-center py-2">
              ...
              <span className="font-medium text-[#002147]">
                {essay.lastModified.toLocaleDateString()}
              </span>
            </div>
